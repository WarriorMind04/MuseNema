...
struct CardsCarrusel<Item: Identifiable, Destination: View>: View {
    let items: [Item]
    let imageURL: (Item) -> String
    let destination: (Item) -> Destination   // 👈 vista destino por ítem
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 0) {
                    ForEach(items, id: \.id) { item in
                        ...
                        .buttonStyle(.plain) // 👈 quita highlight feo
                    }
                }
